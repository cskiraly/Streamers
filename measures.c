...
void reg_chunk_playout(int id, bool b, uint64_t timestamp)
{
	...
	monNewSample(playout_delay, ((int64_t)(tnow.tv_usec + tnow.tv_sec * 1000000UL - timestamp)) / 1000000.0);
}
...
void reg_chunk_receive(int id, uint64_t timestamp, int hopcount)
{
	...
	gettimeofday(&tnow, NULL);
	monNewSample(chunk_delay, ((int64_t)(tnow.tv_usec + tnow.tv_sec * 1000000UL - timestamp)) / 1000000.0);
}
